...
sh <- 2.0
sigma <- 2.0
hist(clt(n, 10000, sh), freq = FALSE, xlim = c(.5,4.5))
curve(dnorm(x, sh, 1/2), add = TRUE)

n <- 36
hist(clt(n, 10000, sh), freq = FALSE, xlim = c(1,3))
...
crapsci(1000, 0.45, 0.55)

```

Confidence intervals of sojourn times 
...
warmup <- 1000
for (i in 1:100) {
  output <- ssq(numJobs + warmup, seeds[i], showOutput = FALSE, saveSojournTimes = TRUE)
  sojourn <- output$sojournTimes[-(1:warmup)]
  ci <- t.test(sojourn, conf.level = 0.95)
  ci_lo[i] <- ci$conf.int[1]
  ci_hi[i] <- ci$conf.int[2]
}


plot(NA, NA, xlim = c(1,100), ylim = c(0,45), bty = "n", xaxt = "n", xlab = "", ylab = "", las = 1)
exact <- 9
abline(h = exact)
for (i in 1:100){
  if(ci_lo[i] > exact | ci_hi[i] < exact){
      segments(i, ci_lo[i], i,  ci_hi[i], col = "red")
    }else{
      segments(i,  ci_lo[i], i,  ci_hi[i])
    }
}
...
```{r}
output <- ssq(10000, 1234567, showOutput = F, saveSojournTimes = T)
sojourns <- output$sojournTimes[-(1:1000)] # remove "warm-up"
acf(sojourns)
```
...
seeds <- sample(10^6:(10^7 - 1), size = 50, replace = FALSE)
ci_lo <- rep(NA, 50) # vector for 50 CI low endpoints
ci_hi <- rep(NA, 50) # vector for 50 CI high endpoints

for (j in 1:50) {
  set.seed(seeds[j]) # Set the seed for each replication
  numJobs <- 250
  warmup <- 1000
  b <- 50
  n <- numJobs/b
  output <- ssq(numJobs + warmup, seeds[j], showOutput = FALSE, saveSojournTimes = TRUE)
  sojourn <- output$sojournTimes[-(1:warmup)]
  batchMeans <- numeric(n)
  
  for (i in 1:n) {
    batch <- sojourn[(1:b) + (b*(i-1))]
    batchMeans[i] <- mean(batch)
  }
  
  ci <- t.test(batchMeans, conf.level = 0.95)
  ci_lo[j] <- ci$conf.int[1]
  ci_hi[j] <- ci$conf.int[2]
}

min = ceiling(min(ci_lo, ci_hi))
max = round(max(ci_lo, ci_hi))
plot(NA, NA, xlim = c(1, 50), ylim = c(min, max), bty = "n", xaxt = "n", xlab = "", ylab = "", las = 1)
exact <- 9
abline(h = exact)
