...
crapsci <- function(nreps, ylo, yhi){
  ...
  for (i in 1:100) {
    ...
    values <- sapply(1:9, function(i) { craps(nreps, showProgress=FALSE) } )
...
ssqci <- function(ylo = 6, yhi = 12){
  batches <- c(64, 128, 256, 512, 1024, 2048)
  for (x in batches) {
    #Create an empty plot with true theoretical value:
    plot(NA, NA, xlim = c(1,100), ylim = c(ylo,yhi), bty = "n", xaxt = "n", xlab = "", ylab = "", las = 1)
    exact <- 9
    ...
    for (i in 1:100) {
      ...
      b <- x
      n <- numjobs/b
      output<- ssq(numjobs + warmup, seeds[i], showOutput = FALSE, saveSojournTimes = TRUE, showProgress = FALSE)
    ...
    plot(xlab = paste(sum, "misses", sep = " "), ylab = paste("b =", batches[1]))
  }
}
